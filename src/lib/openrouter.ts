...
export function getApiKey(): string | null {
  return localStorage.getItem("google_ai_studio_key") || DEFAULT_KEY || null;
}
...
export async function streamChat({
  ...
  if (!apiKey) {
    onError("Chave do Google AI Studio não configurada. Vá em Perfil para adicionar.");
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        onError("Chave do Google AI Studio inválida. Verifique em Perfil. 🔑");
        return;
      }
